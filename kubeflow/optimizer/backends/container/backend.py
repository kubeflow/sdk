    ...
    for param_name, param_spec in search_space.items():
        ...
        if param_type == "categorical":
            ...
            if feasible_space.distribution == "logUniform":
                samples[param_name] = trial.suggest_float(param_name, min_val, max_val, log=True)
            else:
                samples[param_name] = trial.suggest_float(param_name, min_val, max_val, log=False)
        else:
...
class ContainerBackend(RuntimeBackend):
    ...
    def _run_trial(
        ...
        try:
            ...
            actual_job_name = self.trainer_backend.train(
                ...
                runtime=trial_job_template.runtime
                if hasattr(trial_job_template, "runtime")
                else None,
                initializer=trial_job_template.initializer
                if hasattr(trial_job_template, "initializer")
                else None,
        ...
        try:
            ...
            if should_update:
                optimal_trial_data = {
                    ...
                    "observation": {
                        "metrics": [_metric_entry(name, value) for name, value in metrics.items()]
                    },
                }

                storage.update_experiment_status(
                    self.storage_path, job_name, {"currentOptimalTrial": optimal_trial_data}
                )
            ...
            for c in trainjob.steps:
                if c.name == step:
                    yield from self.trainer_backend.get_job_logs(
                        name=trial_name, step=step, follow=follow
                    )
                    break
        except Exception as e:
    ...
    def __get_experiment_from_storage(self, name: str) -> dict:
        ...
        try:
            ...
            raise ValueError(f"Failed to get {constants.OPTIMIZATION_JOB_KIND}: {name}") from e
        except Exception as e:
            raise RuntimeError(f"Failed to load {constants.OPTIMIZATION_JOB_KIND}: {name}") from e

    def __get_optimization_job_from_storage(self, experiment_data: dict) -> OptimizationJob:
    ...
    def __get_trials_from_storage(self, job_name: str) -> list[Trial]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to list {constants.TRIAL_KIND}s for job: {job_name}") from e

...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (SPARK_CONNECT_READY,),
            "get_namespaced_custom_object",
            constants.SPARK_CONNECT_GET_VERBS,
        ),
        ("list_sessions", (), "list_namespaced_custom_object", constants.SPARK_CONNECT_LIST_VERBS),
        (
            "delete_session",
            (SPARK_CONNECT_READY,),
...
def test_403_names_the_required_rbac_rules(kubernetes_backend, method, args, api_method, verbs):
    """A denied request tells the caller which verbs to ask an administrator for.

    Without this the 403 is swallowed by a generic handler and reported as a bare
    "Failed to ..." message, which cannot be distinguished from a broken operator.
    """
    setattr(
        kubernetes_backend.custom_api,
        api_method,
        Mock(side_effect=ApiException(status=403, reason="Forbidden")),
    )

    with pytest.raises(RuntimeError) as exc_info:
        getattr(kubernetes_backend, method)(*args)

    message = str(exc_info.value)
    assert DEFAULT_NAMESPACE in message
    assert constants.SPARK_CONNECT_PLURAL in message
    assert constants.SPARK_CONNECT_GROUP in message
    for verb in verbs:
        assert verb in message
    # The message must be actionable, not merely a denial.
    assert "kind: Role" in message
    assert "kind: RoleBinding" in message


@pytest.mark.parametrize(
    ("method", "args", "api_method", "status"),
    [
        ("get_session", (SPARK_CONNECT_READY,), "get_namespaced_custom_object", 404),
        ("delete_session", (SPARK_CONNECT_READY,), "delete_namespaced_custom_object", 404),
        ("list_sessions", (), "list_namespaced_custom_object", 500),
    ],
)
def test_non_403_errors_are_unchanged(kubernetes_backend, method, args, api_method, status):
    """Only permission failures gain the new message; other errors keep their behaviour."""
    setattr(
        kubernetes_backend.custom_api,
        api_method,
        Mock(side_effect=ApiException(status=status, reason="Boom")),
    )

    with pytest.raises(RuntimeError) as exc_info:
        getattr(kubernetes_backend, method)(*args)

    assert "kind: Role" not in str(exc_info.value)


def test_session_logs_403_names_pod_log_rules(kubernetes_backend):
    """A 403 raised while streaming names pods/log, not sparkconnects.

    read_pod_logs wraps every failure as RuntimeError, so the ApiException is only
    reachable through __cause__.
    """
    kubernetes_backend.get_session = Mock(return_value=Mock(driver_pod_name="driver-0"))

    def _wrapped_403(**kwargs):
        try:
    ...
    with (
        patch(
            "kubeflow.spark.backends.kubernetes.backend.read_pod_logs",
            side_effect=_wrapped_403,
        ),
        pytest.raises(RuntimeError) as exc_info,
    ):
        list(kubernetes_backend.get_session_logs(SPARK_CONNECT_READY))
...
def test_session_logs_non_403_stays_generic(kubernetes_backend):
    """A non-permission streaming failure keeps its existing message."""
    kubernetes_backend.get_session = Mock(return_value=Mock(driver_pod_name="driver-0"))

    def _wrapped_500(**kwargs):
        try:
